...
export interface Props {
  children: {
    minField: (name: string, wrapperClassName: string, placeholder: string) => JSX.Element,
    maxField: (name: string, wrapperClassName: string, placeholder: string) => JSX.Element,
  };
}
  ...
  return (
    <FormGroupWrapper className={'mb-0'}>
      <div className={'form-row'} style={{alignItems: 'baseline'}} id={id}>
        {minField(`${name}.min`, 'flex-grow-1', capitalizeFirstLetter(t('from')))}
        {maxField(`${name}.max`, 'flex-grow-1', capitalizeFirstLetter(t('upto')))}
      </div>
    </FormGroupWrapper>
  );
}
