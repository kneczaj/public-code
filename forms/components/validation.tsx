...
export interface Props extends Pick<ValidationBaseProps<any>, 'ignoreTouched'> {}
...
export const Validation = (props: Props) => {
  ...
  const { meta: { error, submitError, touched } } = fieldProps;
  const displayedError = error || submitError;
  const t = useT();
  const ignoreTouched = isUndefined(props.ignoreTouched) ? ignoreTouchedFromContext : props.ignoreTouched;
  ...
  if (!isUndefined(showErrorWhen) && !showErrorWhen(fieldProps as FieldRenderProps<any, HTMLElement>)) {
    return null;
  }
  return (
    <div className={'invalid-feedback'}>{capitalizeFirstLetter(t(displayedError))}</div>
  );
};
