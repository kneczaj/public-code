...
const valueWithoutWhitespaces = (value: string): string => value.replace(/ /g, '');
 ...
 * @param values
 */
export const isNaturalNumber = skipEmptyField((value: any, values: FormValues) => {
  return validator.isNumeric(String(value), { no_symbols: true }) ? null : 'natural_numbers_only';
});
...
export function oneInFormRequired(value: any, values: FormValues): ValidationResult {
...
export const makeRequired = (label?: string) => (value: any, values: FormValues): ValidationResult => {
...
export const rangeValidator: ValidatorFn = skipEmptyField(
  (value: RangeDataFormat, values: FormValues): ValidationResult => {
    ...
    const aNum: number = Number(value.min);
    const bNum: number = Number(value.max);
...
export const containsOnlyLetters: ValidatorFn = skipEmptyField(
  (value: string, values: FormValues): ValidationResult => {
    return validator.isAlpha(valueWithoutWhitespaces(value), 'pl-PL') ? null : 'accepts only letters';
  }
);

export const isLongerThanOrEqual = (length: number): ValidatorFn => skipEmptyField(
  (value: string, values: FormValues): ValidationResult => {
    return value.length >= length ? null : ['must be longer than', { threshold: length }];
  }
);

export const hasLengthEqual = (length: number, label?: string): ValidatorFn => skipEmptyField(
  (value: string, values: FormValues): ValidationResult => {
    return value.length === length ? null : [label || 'must have length equal', { threshold: length }];
  }
);
...
export const isUniqueKeyword = (others: Array<string>): ValidatorFn => {
  ...
  return skipEmptyField(
    (value: string, values: FormValues): ValidationResult => {
      return lowercaseOthers.includes(value.toLowerCase()) ? 'is already among keywords' : null;
    }
  );
}
...
export const isMobilePhone: ValidatorFn = skipEmptyField(
  (value: string, values: FormValues): ValidationResult => {
    return validator.isMobilePhone(value, 'any') ? null : 'invalid phone number format';
  }
);
...
export const isValidHouseNumber: ValidatorFn = skipEmptyField(
  (value: string, values: FormValues): ValidationResult => {
    ...
    if (validator.isNumeric(value) || (validator.isAlphanumeric(valueWithoutWhitespaces(value), 'pl-PL') && validator.isNumeric(value.slice(0, -1)) && validator.isAlpha(lastChar, 'pl-PL'))) return null;
