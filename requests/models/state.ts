...
export type ListRequestState<TData> = RequestStateBase<TData[]>;
export interface PaginationRequestState<TData>
  extends ListRequestState<TData>,
    PaginationBase {}

type ItemRequestStateBeforeMerge<T> = {
  [K in keyof T]: ItemRequestState<T[K]>;
};
...
export function mergeStates<T>(
  ...
  const inputMap = Map(Object.entries(input)) as Map<
    keyof T,
    ItemRequestState<any>
  >;
  const messages = [...inputMap.values()]
    .map((item: ItemRequestState<any>) =>
      isNull(item.error) ? [] : item.error.messages
    )
    .reduce((acc, val) => [...acc, ...val], []);
  return {
    loading: [...inputMap.values()]
