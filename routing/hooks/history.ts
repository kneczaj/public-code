...
import { CustomLocation, getLocationObj, LocationDescriptorObject, UpdateSearch } from "../models";

export interface Hook<LocationState, Search> extends Omit<History<LocationState>, 'location' | 'push'> {
   ...
   * @param updateSearch
   */
  push: (location: string | LocationDescriptorObject<LocationState>, updateSearch?: UpdateSearch<Search>) => void;
}
 ...
 */
export function useHistory<LocationState, Search extends StringifiableRecord>(): Hook<LocationState, Search> {
  ...
  const search = useMemo(() => parse(searchString) as unknown as Search, [searchString]);
  return {
    ...history,
    location: {
      ...location,
    ...
    push: (location, updateSearch) => {
      history.push(getLocationObj<LocationState, Search>(location, search, updateSearch))
    }
  }
}
